...
from launcher import sorted_morphed_labels_fvs, sorted_genuine4morphed_labels_fvs, \
    sorted_genuine_labels_fvs
...
def get_morphed_genuine_diff_fv(n_images):
    ...
    for el in sorted_morphed_labels_fvs[0:n_images]:
        ...
        genuine_fv = next(
            x[1] for x in sorted_genuine4morphed_labels_fvs if pattern_current_genuine_id.search(x[0]) is not None)
    ...
    for i, el in enumerate(sorted_genuine_labels_fvs):
        ...
        while pattern_current_genuine_id.search(sorted_genuine_labels_fvs[next_i][0]) is not None:
            pairs.append(DifferentialComparison.get_differential_fv(numpy.array(fv),
                                                                    numpy.array(
                                                                        sorted_genuine_labels_fvs[next_i][1])))
            next_i += 1
